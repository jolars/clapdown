...
command is a top-level heading, each subcommand descends exactly one level, and a
command's `Arguments`/`Options` sections sit one level below the command.
...
render natively in [mdBook] (enabled by default) and in Pandoc. The `Flavor` enum
is `#[non_exhaustive]`; `CommonMark` and `Pandoc` flavors are planned.
